...
- **v2.2** — Playtest amendments: staged trump replaceable until `CALL_CARDS` (§9.1/§9.2 — re-staging bumps nothing, emits nothing, leaks nothing; locked by the PHASE-001 v2.2 case); timeout auto-play is now **role-aware** (§10): declarer side uses the team-preserving default (cheapest trick-taking card when the trick holds ≥15 points, else lowest 0-point card of the longest suit, else least-valuable), defenders unchanged (TO-002); declarer-setup phases get extra shell runway before `PAUSED` (`ARCHITECTURE.md` §6, +45s).
