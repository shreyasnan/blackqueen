...
## 12. v2.1 — Creator-selectable hand size (`GAME_SPEC.md` §3.2)

### CONFIG-005 — hand-size bounds, defaults, trim invariants (REQUIRED)
- Bounds: max = whole-deck table value (`maxHandSize`); min = 8 (1 deck) / 10 (2 decks), clamped where the table max is lower (7p/1-deck → only 7 legal).
- Defaults: 1-deck = whole deck (pre-v2.1 behavior, byte-identical deals); 2-deck = 12.
- For EVERY legal `(playerCount, deckCount, handSize)`: trimmed deck has exactly `playerCount × handSize` cards, total point value `150 × deckCount`, and `removedCards` contains no point card.
- Trim order spot check (6p/2-deck/handSize 10): 44 copies removed — all copies of ranks 2/3/4/6/7 (8 each) plus one copy each of 8♣ 8♦ 8♥ 8♠; rank 5 untouched.
- `initGame` rejects out-of-range or non-integer hand sizes; accepts min/max boundaries.
- Deal at a chosen size is deterministic and even.
