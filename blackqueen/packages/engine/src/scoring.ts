...
export const shares = (playerCount: number): number => calledCardCount(playerCount) + 1; // S = C + 1
...
export function pauseEndDelta(playerCount: number, Y: number, declarerSeat: number): number[] {
  return Array.from({ length: playerCount }, (_, seat) => (seat === declarerSeat ? -(shares(playerCount) * Y) : 0));
}
